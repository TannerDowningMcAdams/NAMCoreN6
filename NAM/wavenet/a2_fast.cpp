...
void A2FastModel<Channels>::_layer_forward_fm2(Layer& L, const float* cond, int num_frames)
{
  ...
  // without the per-tap multiply. The ring is exactly max_lookback + nf, so the
  // deepest tap's base - write_pos - nf - max_lookback - is write_pos less one
  // whole ring, and wraps back to write_pos itself. That identity is the ring
  // sizing's, not the kernel's: size the ring any other way and this is wrong
  // by the difference, silently and in range. Frame g0 starts g0 further on,
  // and one wrap is enough because write_pos < ring_size and g0 < nf.
