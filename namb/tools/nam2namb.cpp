...
static void write_wavenet_config(BinaryWriter& w, const json& model_json)
{
  ...
  for (size_t la = 0; la < num_layer_arrays; la++)
  {
    ...
    // Layer-array head. The trainer's current export nests these under "head";
    // legacy .nam files carry head_size/head_bias at the layer-array level with
    // an implicit kernel size and dilation of 1. Mirrors parse_config_json().
    int head_size = 0;
    ...
    {
      const json& head_json = layer["head"];
      if (!head_json.is_object())
        throw std::runtime_error("Layer array " + std::to_string(la) + ": 'head' must be a JSON object");
    ...
    {
      throw std::runtime_error("Layer array " + std::to_string(la)
                               + ": expected 'head' object with out_channels, kernel_size and bias, "
                                 "or legacy 'head_size' and 'head_bias'");
    }
    if (head_kernel_size < 1)
      throw std::runtime_error("Layer array " + std::to_string(la) + ": head.kernel_size must be >= 1");

    // Per-layer kernel sizes. Exactly one of kernel_size (scalar) or
    // kernel_sizes (array) may be present; the scalar broadcasts.
    const bool has_kernel_size = layer.find("kernel_size") != layer.end();
    const bool has_kernel_sizes = layer.find("kernel_sizes") != layer.end();
    ...
    {
      const json& ks_json = layer["kernel_sizes"];
      if (!ks_json.is_array())
        throw std::runtime_error("Layer array " + std::to_string(la) + ": kernel_sizes must be an array");
      for (const auto& ks : ks_json)
        kernel_sizes.push_back(ks.get<int>());
      if (kernel_sizes.size() != num_dilations)
      {
        throw std::runtime_error("Layer array " + std::to_string(la) + ": kernel_sizes size ("
                                 + std::to_string(kernel_sizes.size()) + ") must match dilations size ("
                                 + std::to_string(num_dilations) + ")");
      }
    }
    else if (has_kernel_size)
    {
      kernel_sizes.assign(num_dilations, layer["kernel_size"].get<int>());
    }
    else
    {
      throw std::runtime_error("Layer array " + std::to_string(la)
    ...
    w.write_u16(static_cast<uint16_t>(head_kernel_size)); // v1 held a shared kernel_size here
    ...
    w.write_i32(static_cast<int32_t>(head_dilation)); // v2
    ...
    // Per-layer kernel sizes [num_dilations * int32] (v2)
    for (int ks : kernel_sizes)
      w.write_i32(static_cast<int32_t>(ks));
