...
//
// Version history:
//   v1: Original. WaveNet layer arrays carry a single u16 kernel_size shared by
//       every layer, and no head kernel size or head dilation (both implicitly 1).
//   v2: WaveNet layer arrays carry per-layer kernel_sizes, an explicit head
//       kernel size, and a head dilation. Required for the A2 architecture,
//       whose kernel pattern is 6x14, 15, 15, 6x7 with a K=16 head.
//
// v1 files remain readable: the reader broadcasts the scalar kernel_size across
// all layers and defaults head_kernel_size/head_dilation to 1, which reproduces
// v1 semantics exactly. The writer only emits v2.
//
// WaveNet layer array block, v2 (differences from v1 marked):
//   u16 input_size
//   u16 condition_size
//   u16 head_size
//   u16 channels
//   u16 bottleneck
//   u16 head_kernel_size      <- v1: kernel_size (scalar, per-array)
//   u8  head_bias
//   u8  num_layers
//   u16 groups_input
//   u16 groups_input_mixin
//   i32 head_dilation         <- v2 only
//   ... layer1x1 (4B), head1x1 (6B), 8x FiLM (32B)
//   i32 dilations[num_layers]
//   i32 kernel_sizes[num_layers]   <- v2 only
//   ... activation configs, gating modes, secondary activation configs
...
static constexpr uint16_t MIN_FORMAT_VERSION = 1;
